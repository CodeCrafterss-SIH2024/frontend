...
const Chatbot = () => {
  ...
  return (
    <div>
      <button onClick={toggleChatbot} className="fixed bottom-4 right-4 bg-blue-600 text-white p-4 rounded-full shadow-lg hover:bg-blue-700 transition">
      ...
      {isOpen && (
        <div className="fixed bottom-20 right-4 bg-white border border-gray-300 rounded-lg shadow-lg w-80 h-96 flex flex-col">
          ...
          <div className="flex-1 p-4 overflow-y-auto">
            {chat.map((message, index) => (
              <div key={index} className={`mb-2 ${message.type === 'user' ? 'text-right' : 'text-left'}`}>
                <div className={`inline-block p-2 rounded-lg ${message.type === 'user' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800'}`}>
                  {message.text}
                </div>
              </div>
            ))}
          ...
          <div className="p-4 bg-gray-100 rounded-b-lg">
            ...
            />
            <button onClick={handleSend} className="mt-2 w-full bg-blue-600 text-white p-2 rounded-lg hover:bg-blue-700 transition">Send</button>
          </div>
        </div>
      )}
    </div>
