...
const Crossword = ({ userId }) => {
  ...
  const checkPuzzle = async () => {
    ...
    if (correct) {
      ...
      setResult('Congratulations! You have Completed the Crossword correctly and Earn 100 coins !!');
      ...
      setResult('Some answers are incorrect. Keep trying!');
    }
  };
  ...
  return (
    <div className="crossword-container p-8 text-center">
      ...
      <div className="max-w-md mx-auto text-left mt-8 bg-white p-6 border border-gray-200 rounded-lg shadow-lg">
        <h2 className="text-4xl font-semibold text-blue-800 mb-6 text-center">Clues</h2>
        <div className="mb-6">
          <h3 className="text-2xl font-bold text-blue-600 mb-3">Across</h3>
          <ul className="list-disc pl-5 text-gray-800">
            {acrossClues.map((clue, i) => (
              <li key={i} className="mb-2">{clue}</li>
            ))}
        ...
        <div>
          <h3 className="text-2xl font-bold text-blue-600 mb-3">Down</h3>
          <ul className="list-disc pl-5 text-gray-800">
            {downClues.map((clue, i) => (
              <li key={i} className="mb-2">{clue}</li>
            ))}
      ...
      <p className={`mt-6 text-lg ${result.startsWith('Congratulations ') ? 'text-green-600' : 'text-[#dbdce5]'}`}>
        {result}
      </p>
    </div>
  );
